	...
	#pragma omp parallel
		...
		for(int i=1; i<mField->getNpx()-1; i++){
			for(int j=0; j<mField->getNpy()-1; j++){
				HX(i, j, +1) = HX(i, j, 0) - CHXLYNS(i, j)*(EZ(i, j + 1, +1) - EZ(i, j, +1));	//Hxの計算 Hx(i, j+1/2) -> Hx[i,j]
			}
		}
		...
		for(int i=0; i<mField->getNpx()-1; i++){
			for(int j=1; j<mField->getNpy()-1; j++){
				HY(i, j, +1) = HY(i, j, 0) + CHYLXNS(i, j)*(EZ(i + 1, j, +1) - EZ(i, j, +1));	//Hyの計算 Hy(i+1/2, j) -> Hy[i,j]
			}
		}
			...
			for (int i = 1; i<mField->getNpx()-1; i++) {
				for (int j = 0; j<mField->getNpy() - 1; j++) {
					if (i == 1 || i == mField->getNpx() - 2 || j == 0 || j == mField->getNpy() - 2)
						...
						HX(i, j, +1) = BHYP(i, j) * BHYM(i, j) * HX(i, j, 0) - BHYP(i, j)*CHXLYNS(i, j)*(R_P*(EZ(i, j+1, +1) - EZ(i, j, +1)) + R_M*Dy2_n(Ez, i, j+1, +1));	//Hxの計算 Hx(i, j+1/2) -> Hx[i,j]
																									   //*Dy0_n(Ez, i, j, +1);
				}
			}
			...
			for (int i = 0; i<mField->getNpx() - 1; i++) {
				for (int j = 1; j<mField->getNpy()-1; j++) {
					if (j == 1 || i == mField->getNpy() - 2 || i==0 || i==mField->getNpx()-2)
						...
						HY(i, j, +1) = BHXP(i, j) * BHXM(i, j) * HY(i, j, 0) + BHXP(i, j)*CHYLXNS(i, j)*(R_P*(EZ(i+1, j, +1) - EZ(i, j, +1)) + R_M*Dx2_n(Ez, i+1, j, +1));	//Hyの計算 Hy(i+1/2, j) -> Hy[i,j]
																									   //*Dx0_n(Ez, i, j, +1);
				}
			}
